...
void check(pii origin, pii dest) {
    ...
    if (dest.first < 1 || dest.first > N || dest.second < 1 || dest.second > M) return;
    if (paths[dest.first][dest.second] == 0) return;
...
void bfs() {
    while (!q.empty()) {
        ...
        q.pop();
        next = {loc.first + 1, loc.second};
        check(loc, next);

        next = {loc.first - 1, loc.second};
        check(loc, next);

        next = {loc.first, loc.second + 1};
        check(loc, next);
